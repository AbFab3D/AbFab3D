 ...
 * @version 0.8
 */
public class RTextMenuBar extends MenuBar implements PopupMenuListener, ProjectListener {
	...
	private JMenuItem newVariantItem;
	...
	private JMenuItem saveAllVariantModelsItem;
	...
	private JMenuItem resetViewItem;

	private JMenu fileMenu;
	private JMenu viewMenu;
	private JMenu windowMenu;
	private UtilitiesFilesMenu utilsFilesMenu;
	 ...
	 */
	private final int MAX_FILE_PATH_LENGTH = 500;
	...
	public RTextMenuBar(final RText rtext, String lnfName,
		...
		ResourceBundle menuMsg = ResourceBundle.getBundle("ide.MenuBar");
		...
		JMenu newMenu = createMenu(menuMsg, "MenuNew");
		fileMenu.add(newMenu);

		// File -> New submenu items
		newProjectItem = createMenuItem(rtext.getAction(RText.NEW_PROJECT_ACTION));
		newMenu.add(newProjectItem);
		newScriptItem = createMenuItem(rtext.getAction(RText.NEW_SCRIPT_ACTION));
		newMenu.add(newScriptItem);
		newVariantItem = createMenuItem(rtext.getAction(RText.NEW_VARIANT_ACTION));
		newMenu.add(newVariantItem);
		...
		saveAllVariantModelsItem = createMenuItem(rtext.getAction(RText.SAVE_ALL_VARIANT_MODELS_ACTION));
		fileMenu.add(saveAllVariantModelsItem);

		exportProjectItem = createMenuItem(rtext.getAction(RText.EXPORT_PROJECT_ACTION));
		fileMenu.add(exportProjectItem);

//		exportAsItem = createMenuItem(rtext.getAction(RText.EXPORT_AS_ACTION));
//		fileMenu.add(exportAsItem);
//		fileMenu.addSeparator();

/*      // Removed the print actions as I doubt they will be used
...
*/
		...
		utilsFilesMenu = new UtilitiesFilesMenu(menuMsg.getString("UtilitiesFiles")) {
			@Override
			protected Action createOpenAction(String fileFullPath) {
				OpenFileFromHistoryAction action =
						new OpenFileFromHistoryAction(rtext);

				String projName = FilenameUtils.getFullPathNoEndSeparator(fileFullPath);
				int idx = projName.lastIndexOf(File.separator);
				if (idx != -1) projName = projName.substring(idx+1);

				action.setName(projName);
				action.setFileFullPath(fileFullPath);
				return action;
			}
		};
		fileMenu.add(utilsFilesMenu);
		...
		Dimension dim = recentFilesMenu.getPreferredSize();
		Dimension pdim = fileMenu.getPreferredSize();
...
//		UIUtil.setDescription(lineNumbersItem, msg, "DescLineNumbers");
		...
		// Run submenu's items.
		runItem = createMenuItem(rtext.getAction(RText.RUN_ACTION));
		menu.add(runItem);
		
        resetViewItem = createMenuItem(rtext.getAction(RText.RESET_VIEW_ACTION));
	...
	public void addNotify() {
		...
		if (initialFileHistoryString!=null && initialFileHistoryString.length()>0) {
			...
			if (initialContents.length > 0) {
				rtext.setLastOpenProjectFile(initialContents[initialContents.length - 1]);
			}
	...
	private String getDisplayPath(String longPath) {
		...
		// Keep first directory, then go from front

		int pdirIdx = displayString.lastIndexOf(separator,lastSeparatorPos-1);
		if (pdirIdx == -1) return displayString;
		String pdirFileName = displayString.substring(pdirIdx+1,displayString.length());
		int pdirFileNameLength = getTextWidth(pdirFileName, fontMetrics);
		if (pdirFileNameLength > MAX_FILE_PATH_LENGTH) {
			return "..." + separator + justFileName;
		}

		justFileName = pdirFileName;
	...
	protected void menuItemAcceleratorWorkaround() {
		...
		updateAction(newProjectItem, RText.NEW_PROJECT_ACTION);
		updateAction(newScriptItem, RText.NEW_SCRIPT_ACTION);
		updateAction(newVariantItem, RText.NEW_VARIANT_ACTION);
		...
		updateAction(saveAllVariantModelsItem, RText.SAVE_ALL_VARIANT_MODELS_ACTION);
//		updateAction(exportAsItem, RText.EXPORT_AS_ACTION);
