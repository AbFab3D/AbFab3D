...
import ide.RText;
import ide.RTextActionInfo;
import ide.RTextPrefs;
...
public class ActionFactory implements RTextActionInfo {
	...
	public static void addActions(RText rtext, RTextPrefs prefs) {
		...
		String commonIconPath = "ide/graphics/common_icons/";
		String baseIconPath = "ide/graphics/";

		try {
			rtext.setIconImage(ImageIO.read(cl.getResource(
						"ide/graphics/rtexticon.gif")));
		} catch (IOException ioe) {
			ioe.printStackTrace();
		}
		
		AppAction<RText> a = new NewProjectAction(rtext, msg, null);
		rtext.addAction(NEW_PROJECT_ACTION, a);
		
		a = new NewScriptAction(rtext, msg, null);
		rtext.addAction(NEW_SCRIPT_ACTION, a);
		
		a = new NewVariantAction(rtext, msg, null);
		rtext.addAction(NEW_VARIANT_ACTION, a);
		...
		a = new SaveAllVariantModelsAction(rtext, msg, null);
		rtext.addAction(SAVE_ALL_VARIANT_MODELS_ACTION, a);

		a = new ExportProjectAction(rtext, msg, null);
		rtext.addAction(EXPORT_PROJECT_ACTION, a);
		...
		a = new RunAction(rtext, msg, new ImageIcon(cl.getResource(baseIconPath+"reload_16x16.png")));
		rtext.addAction(RUN_ACTION, a);
		
        a = new ResetViewAction(rtext, msg, new ImageIcon(cl.getResource(baseIconPath+"reset_view_16x16.png")));
        rtext.addAction(RESET_VIEW_ACTION, a);
	}
