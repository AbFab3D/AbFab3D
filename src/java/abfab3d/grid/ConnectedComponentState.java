...
*/
public class ConnectedComponentState implements Comparable, Region {
    ...
    byte state; // material  of the component
    Grid grid;
    ...
    */
    public ConnectedComponentState(Grid grid, GridBit mask, int x, int y, int z, byte state, boolean collectData){
        this(grid, mask, x, y, z, state, collectData, DEFAULT_ALGORITHM);
    }

    public ConnectedComponentState(Grid grid, GridBit mask, int x, int y, int z, byte state, boolean collectData, int algorithm){
    ...
    boolean fillScanLineQueue(int start[]) {
        ...
        while(!que.isEmpty()) {
            ...
            while((y1 <= ny1) && compareState(grid,x,y1,z,state) ){
                ...
                // check x-direction 
                if(!spanxLeft && (x > 0) &&  (mask.get(x-1, y1, z) == 0) && compareState(grid,x-1,y1,z,state)){
                ...
                } else if(spanxLeft && (x > 0) && ((mask.get(x-1, y1, z)!= 0) || !compareState(grid,x-1,y1,z,state))){
                ...
                if(!spanxRight && x < nx1 &&  (mask.get(x+1, y1, z)==0)  && compareState(grid,x+1,y1,z,state)) {
                    // start of potential new span
                ...
                } else if(spanxRight && x < nx1 && ((mask.get(x+1, y1, z)!= 0) || !compareState(grid,x+1,y1,z,state))) {
                    // end of potential new span
                ...
                // check z direction 
                if(!spanzLeft && (z > 0) &&  (mask.get(x, y1, z-1)==0) && compareState(grid, x,y1,z-1, state)){
                    // start of potential new span
                    //que.offer(new int[]{x, y1, z-1});
                    que.offer(x, y1, z-1);
                    spanzLeft = true;
                    
                } else if(spanzLeft && (z > 0) && ((mask.get(x, y1, z-1)!= 0) || !compareState(grid,x,y1,z-1,state))){
                    // end of potential new span
                ...
                if(!spanzRight && z < nz1 &&  (mask.get(x, y1, z+1)==0)  && compareState(grid, x,y1,z+1,state)) {
                    // start of potential new span                        
                    //que.offer(new int[]{x, y1, z+1});
                    que.offer(x, y1, z+1);
                    spanzRight = true;
                    
                } else if(spanzRight && z < nz1 && ((mask.get(x, y1, z+1)!= 0) || !compareState(grid,x,y1,z+1,state))) {
                    // end of potential new span
                    ...
                    for(int n2=-1; n2 < 2; n2++) {
                        for(int n3=-1; n3 < 2; n3++) {
                            ...
                            if (mask.get(ni,nj,nk) == 0) {
                                ...
                                if (!compareState(grid, ni, nj, nk,state))
                                    continue;
