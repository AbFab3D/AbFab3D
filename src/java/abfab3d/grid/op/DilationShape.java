    ...
    static class SurfaceFinder implements ClassTraverser {
        ...
        */ 
        void processVoxel(int x,int y,int z){
            ...
            if(grid.getState(x+1,y,z) == OUTSIDE) surface.set(x,y,z,1);
            if(grid.getState(x-1,y,z) == OUTSIDE) surface.set(x,y,z,1);
            if(grid.getState(x,y+1,z) == OUTSIDE) surface.set(x,y,z,1);
            if(grid.getState(x,y-1,z) == OUTSIDE) surface.set(x,y,z,1);
            if(grid.getState(x,y,z+1) == OUTSIDE) surface.set(x,y,z,1);
            if(grid.getState(x,y,z-1) == OUTSIDE) surface.set(x,y,z,1);
