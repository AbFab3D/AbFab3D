...
public class SceneImageDataSource extends BaseParameterizable implements DataSource, Initializable {
    ...
    */
    public int getDataValue(Vec pnt, Vec dataValue) {
        ...
        if(!intersectBox(m_eyeOrigin, direction, boxMin(), boxMax(), intersection)){
            ...
            Vector4d color = raytracePixel(intersection[0], intersection[1], m_eyeOrigin, direction);
    ...
    */
    ...
    Vector4d raytracePixel(double tnear, double tfar, Vector3d rayOrigin, Vector3d rayDirection) {

        Vector3d pos_world = new Vector3d();
        Vector3d normal = new Vector3d();  // surface normal at intersection
        ...
        if(false)printf("pos_box: [%5.2f, %5.2f, %5.2f]\n", pos_box.x,pos_box.y,pos_box.z);
        // got surface intersection 
        MaterialData mat = getMaterial(data);
        Vector4d color = shadeSurface(pos_world, pos_box, normal, rayDirection, data);
    ...
    */
     ...
     */
    Vector4d shadeSurface(Vector3d posWorld, Vector3d posBox, Vector3d normal, Vector3d eyeRayD, Vec data) {
        ...
        return getPhongShading(material,color,posWorld,posBox,normal,eyeRayD,data);
    ...
    //
    // Calculate the reflection of vector v in vector plane with normal n
    //
    Vector3d reflect(Vector3d v, Vector3d n) {

        return sub(v, mul(n,2*dot(n,v)));        
    ...
    Vector4d m_backgroundColor = new Vector4d(0,0,0,1);
    ...
    */
    ...
    static class MaterialData {
        ...
        MaterialData(PhongParams params){
            
            materialType = params.getMaterialTypeIndex();
            diffuseColor = vec4(params.getDiffuseColor());
            emissiveColor = vec4(params.getEmissiveColor());
            specularColor = vec4(params.getSpecularColor());
            albedo = vec4(params.getAlbedo());
            roughness = params.getRoughness();
            shininess = params.getShininess();
            ambientIntensity = params.getAmbientIntensity();
