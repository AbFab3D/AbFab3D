 ...
 * @version
 */
public class TestBlockArrayTwoBitGrid extends BaseTestGrid implements ClassTraverser {
	...
	public void testTwoBitArray() {
		...
		for (int i = 0; i < 5; i++) {
			...
			twob.set(i*4+1, Grid.EXTERIOR);
			twob.set(i*4+2, Grid.INTERIOR);
			twob.set(i*4+3, Grid.USER_DEFINED);
		}
		for (int i = 0; i < 5; i++) {
			assertEquals("State should be ", Grid.OUTSIDE, twob.get(i*4+0));
			assertEquals("State should be ", Grid.EXTERIOR, twob.get(i*4+1));
			assertEquals("State should be ", Grid.INTERIOR, twob.get(i*4+2));
			assertEquals("State should be ", Grid.USER_DEFINED, twob.get(i*4+3));
		}
	}
...
//		assertEquals("State should be ", Grid.INTERIOR, grid.getState(0.1, 0.1, 0.15));
	...
	protected static void setX(Grid grid, int y, int z, byte state, byte mat, int startIndex, int endIndex) {
		for(int x=startIndex; x <= endIndex; x++) {
			((BlockArrayTwoBitGrid) grid).setState(x,y,z, state);
		}
	}
